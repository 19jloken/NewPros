...
int position = 0;
int color;
int liftLabel = 4095 - int(getLiftSensor());
bool gyro = 0;
...
static lv_res_t btnm_action(lv_obj_t * btnm, const char *txt) {
  ...
  case 2:
    ...
    break;
  case 3:
  lv_label_set_text(g_sb_label, "Red Back Cap Auton");
  lv_label_set_text(autonTxt, "Gets ball from cap by platforms\n"
    "Picks up cap and scrores on post\n"
    "Gets other cap and scores it then parks");
    auton_sel = 3;
    lv_obj_set_style(scr3, &red);
    lv_scr_load(scr2);
    break;
  ...
  lv_label_set_text(autonTxt, "Gets ball from cap by platforms\n"
    ...
    break;
  case 6:
  lv_label_set_text(g_sb_label, "Blue Back Cap Auton");
  lv_label_set_text(autonTxt, "Gets ball from cap by platforms\n"
    "Picks up cap and scrores on post\n"
    "Gets other cap and scores it then parks");
    auton_sel = 6;
...
static lv_res_t left(lv_obj_t * rightButton)
{
  ...
  {
    auton_sel = 7; //the value is set back to the top so the list appears to forever be able to scroll through
  }
  else
  {
...
static lv_res_t right(lv_obj_t * leftButton)
{
  ...
  if (auton_sel == 7) //if int is at 6 it has reached the top value of the list
  {
...
static lv_res_t zeroDrive(lv_obj_t *zeroDrive)
{
...
void label_refresher_lift(void * p) //begins the loop that will autoupdate the left encoder label
{
  static uint32_t prev_value = 0;

  if(prev_value != getLiftSensor())
  {
    if(lv_obj_get_screen(liftValue) == scr3)
    {
      char buf[32];
      sprintf(buf, "Lift: %d", int(getLiftSensor()));
      lv_label_set_text(liftValue, buf);
    }
    prev_value = getLiftSensor();
  }
}
...
void gui(void)
{
    // Create a new screen and load it
     ...
     lv_obj_set_pos(zeroDrive, 325, 65);
     ...
     liftValue = lv_label_create(scr3, NULL);
     lv_obj_set_pos(liftValue, 10, 130);
     lv_obj_set_style(liftValue, &style_txt);
     lv_task_create(label_refresher_lift, 100, LV_TASK_PRIO_MID, NULL);
